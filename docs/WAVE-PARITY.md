...
| 1.4 | Automatic late-payment reminders (Pro) (waveapps.com/invoicing, /pricing: "Automate late payment reminders") | **MATCHED** (scheduler built this session) | Per-client schedules — before due, on the due date, and a past-due ladder — set on the client record and run once a day by a Vercel cron hitting `app/api/reminders/run/route.ts`; the same pass is a button in Settings → Reminders, so the feature works with no scheduler configured at all. Decision logic is pure and tested (`lib/reminders/policy.ts`, `tests/reminder-schedule.test.mjs`: DST, month-end, leap day, supersede, quiet periods); sending reuses the existing reminder path (`lib/email/send-invoice.ts` → `buildReminderMessage`), so saved templates and `{{days_overdue}}` work unchanged. Schema: `20260813130000_payment_reminders_and_late_fees.sql`. **Where it is deliberately narrower than Wave:** one reminder per invoice per run, ever (a ladder switched on for a 40-day-late invoice sends the 30-day rung and records the rest as skipped, not four emails); a five-day quiet period after ANY reminder including a manual one; a pause when the client has just opened the share link; and per-invoice "leave this one alone" (`invoices.reminders_suppressed`). Every attempt is recorded with its true outcome in `pilot.invoice_reminder_sends` — a refused send is a `failed` row that surfaces in Overview's attention queue, never a pretend send. Off for every client until a pilot sets a schedule, and dormant on any deployment without `CRON_SECRET`. *Same operational caveat as 1.1:* real sends are blocked until the Resend domain is verified (G5); a run then records the 403 rather than lying about it. |
